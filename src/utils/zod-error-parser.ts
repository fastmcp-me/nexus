...
/**
 * Enhanced Zod error parsing for user-friendly validation messages
 */
export class ZodErrorParser {
  /**
  ...
  static parseZodError(error: ZodError): {
    ...
    const details = error.issues.map(issue => this.parseZodIssue(issue));
   ...
   */
  private static parseZodIssue(issue: ZodIssue): {
    field: string;
    issue: string;
    received?: unknown;
    expected?: string | string[];
    ...
    switch (issue.code) {
      ...
      case 'invalid_enum_value':
        return this.handleInvalidEnum(field, issue);

      case 'too_small':
        return this.handleTooSmall(field, issue);

      case 'too_big':
        return this.handleTooBig(field, issue);
  ...
  private static handleInvalidEnum(
    ...
    if (field === 'model') {
      const suggestion = this.suggestModelName(String(received));
      return {
        field,
   ...
   */
  private static suggestModelName(input: string): string | null {
   ...
   */
  static createUserFriendlyMessage(error: unknown): {
    message: string;
    ...
    if (this.isZodError(error)) {
      const parsed = this.parseZodError(error);
      return {
